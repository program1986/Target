...
BEGIN_MESSAGE_MAP(CTargetDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_ACCESS_FILESYSTEM, &CTargetDlg::OnBnClickedAccessFilesystem)
END_MESSAGE_MAP()
...
//访问文件系统操作
void CTargetDlg::OnBnClickedAccessFilesystem()
{
	// TODO: 在此添加控件通知处理程序代码
	CString log = CString("hello");
	...
	CString xxx;
	GetDlgItemText(IDC_LOGOUT, xxx);
	xxx = xxx + log + CString("\r\n");
	SetDlgItemText(IDC_LOGOUT, xxx);  

	HANDLE pFile;
	char* tmpBuf;
	DWORD dwBytesWrite, dwBytesToWrite;

	pFile = CreateFile(L"a.txt", GENERIC_WRITE,
		0,
		NULL,
